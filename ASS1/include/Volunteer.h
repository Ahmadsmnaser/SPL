...
public:
    ...
    bool getrecentlyFinished() const;
    void setrecentlyFinished(bool recentlyfinish);
    void setActiveOrder(int id);
    ...
    virtual void acceptOrder(const Order &order) = 0;        // Prepare for new order(Reset activeOrderId,TimezLeft,DistanceLeft,OrdersLeft depends on the volunteer type)
    ...
    virtual void volunteerStatus() = 0;
    const string isBusyPrint() const;
    void setType(VolunteerType toset);
    VolunteerType getType();
...
public:
    ...
    //virtual CollectorVolunteer(const CollectorVolunteer& other) override;
...
public:
    ...
    //virtual ~LimitedDriverVolunteer() = default; // Virtual destructor
